...
export default function HomeScreen({ navigation }: { navigation: any }) {
    ...
    try {
      ...
      const users = patientQuerySnapshot.docs.map((doc) => {
        ...
        const birthDate = birthTimestamp
          ? new Date(birthTimestamp.seconds * 1000)
          : null;
  ...
  const handleVideoClick = async (url: string) => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        "An unexpected error occurred while trying to open the video"
      );
    }
  };
    ...
    <View style={styles.videoItem}>
      ...
      <View style={styles.videoInfo}>
        ...
        <TouchableOpacity onPress={() => toggleFavorite(item.uid)}>
          <MaterialIcons
            name={
              favoriteVideos.includes(item.uid) ? "favorite" : "favorite-border"
            }
            size={24}
        ...
        <View style={styles.userInfoContainer}>
          ...
          <View style={styles.userDetails}>
            ...
            <Text style={styles.userName}>
              {userData?.firstName || null} {userData?.secondName || null}
            </Text>
            <Text style={styles.userAge}>
              {userData?.dateOfBirth
                ? `${userData.dateOfBirth} años`
                : `${0} años`}
            </Text>
            <TouchableOpacity
